...
const EditAccount2 = () => {
    ...
    const resend = () => {
        api.post(`api/edit_details1/${route.params.user.id}/`, {
                            email: JSON.stringify(route.params.email),
                            action: 'send'
                        })
                        .then(res => {
                            Alert.alert(
    ...
    const checkValid = () => {
        console.log('called')
        api.get(`api/user/${route.params.user.id}/`)
        .then(res => {
    ...
    return (
        <View style={{flex: 1, backgroundColor: 'white'}}>
            ...
            <Text style={styles.title1}>Você precisará verificar seu novo email para salvá-lo.</Text>
            <Image style={styles.image} source={require('../assets/propImages/mail.png')}/>
            {!verified ? 
            <View style={{paddingHorizontal: 15}}>
            <Text style={styles.text}>Foi enviado um email para <Text style={{fontWeight: '700'}}>{route.params.email}</Text> com um link para verificar o seu email. Se você não recebeu o email depois de alguns minutos, você pode pressionar "Reenviar email".
        ...
        <View>
        <View style={styles.button3}><View style={styles.side}></View><View style={styles.verified}><Text style={styles.veri}>O seu endereço de email foi verificado com sucesso. Pressione "Salvar" para salvar seu novo email.</Text></View></View>
